...
export class OfflineDataAdapter {
  ...
  getCollection(collection, tenantId = null) {
    ...
    if (tenantId && Array.isArray(list)) {
      return list.filter(item => !item.tenantId || item.tenantId === tenantId);
    }
    return list;
  }
  ...
  getById(collection, id, tenantId = null) {
    ...
    return list.find(item => item.id === id || item.uuid === id || item.code === id || item.itemCode === id || item.categoryCode === id || item.supplierCode === id || item.uomCode === id || item.locationCode === id || item.poNumber === id || item.grnNumber === id) || null;
  }
  ...
  update(collection, id, patch, session = null) {
    ...
    const idx = list.findIndex(item => (item.id === id || item.uuid === id || item.code === id || item.itemCode === id || item.categoryCode === id || item.supplierCode === id || item.uomCode === id || item.locationCode === id || item.poNumber === id || item.grnNumber === id) && (!tenantId || item.tenantId === tenantId));
    if (idx !== -1) {
  ...
  delete(collection, id, session = null) {
    ...
    const tenantId = session ? session.tenantId : '';
    const list = this.store.getCollection(collection) || [];
    const filtered = list.filter(item => !( (item.id === id || item.uuid === id || item.code === id || item.itemCode === id || item.categoryCode === id || item.supplierCode === id || item.uomCode === id || item.locationCode === id || item.poNumber === id || item.grnNumber === id) && (!tenantId || item.tenantId === tenantId) ));
